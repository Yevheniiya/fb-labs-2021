...
# Функція отримання (оброблює отримане повіжомлення, розшифровуючи його та перевіряючи його піддліність і )
...
[[Bob_n, Bob_e], [Bob_d, Bob_p, Bob_q]] = generate_keys(1025)  # Bob's keys
print("Keys of Alice:")
...
# signing and verifying with server ( Alice sends signed message to server)
public_key_request = f"http://asymcryptwebservice.appspot.com/rsa/serverKey?keySize=1025"
...
send_public_key = f"http://asymcryptwebservice.appspot.com/rsa/receiveKey?key={signed_text[0]}" \
                  f"&signature={signed_text[1]}&modulus={hex(Alice_n)[2:]}&publicExponent={hex(Alice_e)[2:]}"
